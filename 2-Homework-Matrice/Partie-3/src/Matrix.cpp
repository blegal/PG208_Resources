...
template<class T>
int Matrix<T>::get(const int x, const int y) const 
{
	assert( x < width () );
	assert( y < height() );
...
template<class T>
void Matrix<T>::set(const int x, const int y, const int value)
{
	assert( x < width () );
	assert( y < height() );
...
template<class T>
void Matrix<T>::add(const int value)
{
	for(int y = 0; y < height(); y++){
		for(int x = 0; x < width(); x++){
...
template<class T>
void Matrix<T>::sub(const int value)
{
	add( -value );
}
...
template<class T>
void Matrix<T>::mult(const int value)
{
	for(int y = 0; y < height(); y++){
		for(int x = 0; x < width(); x++){
			set(x, y, get(x, y) * value);
